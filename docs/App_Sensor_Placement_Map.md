...
- 센서 부착 안내 화면에는 `화면 표시명`과 `부착 위치`를 한국어로 표시한다.
- 앱 내부에서 운동별 센서 위치를 조회할 때는 `exercise_type`을 기준으로 한다.
- 결과 화면과 히트맵에서는 `muscle_map`의 영어 key를 한국어 표시명으로 변환해서 보여준다.
- 운동 중에는 고주기 센서 수치를 직접 표시하지 않고, 상태 이벤트와 제어 버튼 중심으로 화면을 유지한다.

### 9-2. Pi

- 앱에서 `submit_workout_plan`을 받은 뒤 운동 계획을 검증한다.
- 앱에서 `start_calibration`을 받으면 현재 운동의 센서 매핑 기준으로 캘리브레이션을 수행한다.
- 운동 종료 후 `session_result.muscle_map`에 이 문서의 대표 key를 사용해 요약 결과를 담는다.
- Pi 내부 채널 번호와 앱 문서의 EMG/IMU 채널 번호가 다르게 해석되지 않도록 유지한다.

### 9-3. 서버

- `session_result` 저장 시 `muscle_map`의 key를 이 문서 기준으로 보존한다.
- 통계/히트맵 API에서 같은 key를 사용해 앱이 별도 변환 없이 해석할 수 있게 한다.
- 화면 표시용 한글명은 서버 응답에 강제하지 않고, 앱의 표시 매핑에서 처리한다.

### 9-4. 스마트글래스

- 글래스 HUD는 앱과 같은 채널 의미를 기준으로 상태 안내를 표시한다.
- 운동 중 상세 수치 전체를 표시하기보다 캘리브레이션 상태, 주의 안내, 운동 진행 상태를 우선 표시한다.

## 10. 완료 기준

이 문서는 아래 조건을 만족하면 15번 작업의 기준 문서로 본다.

- 공통 EMG/IMU 채널 규칙이 정의되어 있다.
- `pushup`, `bicep_curl`, `lateral_raise`의 센서 부착 위치가 정의되어 있다.
- 화면 표시명과 데이터 key가 분리되어 있다.
- 앱/Pi/서버가 공통으로 사용할 `muscle_map` 대표 key가 정리되어 있다.
- 이 기준이 실시간 스트림 UI가 아니라 센서 부착 안내와 `session_result.muscle_map` 해석 기준임을 명시한다.
