...
| `gyro_x` | `int16_t` | 2 | 자이로 X축, 실수값에 `100` 스케일 적용 |
| `gyro_y` | `int16_t` | 2 | 자이로 Y축, 실수값에 `100` 스케일 적용 |
| `gyro_z` | `int16_t` | 2 | 자이로 Z축, 실수값에 `100` 스케일 적용 |
...
- `acc_*`는 `1000` 스케일, `gyro_*`는 포화 방지를 위해 `100` 스케일을 사용합니다.
